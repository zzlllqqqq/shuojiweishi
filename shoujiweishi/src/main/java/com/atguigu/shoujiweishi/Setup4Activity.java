...
import com.atguigu.shoujiweishi.receiver.MyAdminReceive;
...
public class Setup4Activity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //如果已经保存开启保存, 更新cb_setup4
        boolean protect = SPUtils.getInstance(this).getValue(SPUtils.PROTECT, false);
        if (protect) {
    ...
    private boolean isActive() {
        ...
        ComponentName name = new ComponentName(this, MyAdminReceive.class);
    ...
    public void confirm(View v) {
        ...
        //如果没有激活, 需要启动激活
        if (!isActive()) {
            ComponentName componentName = new ComponentName(this, MyAdminReceive.class);
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == 1 && resultCode == RESULT_OK) {//激活
            //保存完成配置的标识
            SPUtils.getInstance(this).save(SPUtils.CONFIGURE, true);
            //跳转到信息界面
            Intent intent = new Intent(this, ProtectinfoActivity.class);
            startActivity(intent);
            finish();
        } else if (requestCode == 1 && resultCode == RESULT_CANCELED) {//取消
